				...
				if (resp.ok) 
				{
					...
					if (result.statuscode === 0)
				    {
					...
					else if (result.statuscode === 1)
					 {
						...
						// sessionStorage.setItem("pname",JSON.stringify(result.udata.name));
						// sessionStorage.setItem("uname",JSON.stringify(result.udata.name));
						if (rembme === true) {
				...
				<div className="container">
					...
					<div className="login-form-grids animated wow slideInUp" data-wow-delay=".5s">
						<form name="form1" method="post" onSubmit={onlogin}>
							...
							<ReCAPTCHA sitekey="6Lfu4GspAAAAABUTxFPhXmM1Wm1H3wb4yfh7jy5H" onChange={onCaptChange} />
							<label>
								<input type="checkbox" name="remb" onChange={(e)=>setrembme(e.target.checked)}/>Remember Me	</label>
