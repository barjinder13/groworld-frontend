...
var ShowCart = () => {
    ...
    useEffect(()=>
    {
        ...
        {
            totcost = totcost + cartdata[x].tcost; // 400+707.52
        }
        setbillamt(totcost);
        ...
        try {
            ...
            if (resp.ok) {
                ...
                if (result.statuscode === 0) {
                    // toast.error("No Product added in cart yet");
                    setcartdata([]);
                    setflag(false);
                }
    ...
    if (uchoice === true)
    {
            ...
            if (resp.ok) {
                ...
                if (result.statuscode === 1) {
                    toast.success("Item Deleted from cart Successful");
                    fetchcart(); 
                }
                                ...
                                <table className="timetable_sub">
                                    <tbody>
                                        ...
                                        </tr>
                                        {
                                            cartdata.map((data,i)=>
                                            <tr key={i}>
                                                <td><img height='75' alt="Cart" src={`uploads/${data.picture}`} /></td>
                                                <td>{data.pname}</td>
                                                <td>{data.rate}</td>
                                                <td>{data.qty}</td>
                                                <td>{data.tcost}</td>
