...
var ManageSubCategory = () => {
    ...
    var onupdate=(scid)=> 
    {
        //  navigate (`/uploadssubcategory?subcatid=${scid}`);
        navigate({
            pathname: '/updatesubcategory',
            search: `?subcatid=${scid}`,
        });
    }
    var delsubcat=async(id)=>
    {
      var ucchoice = window.confirm("Are you sure to delete?");
      if (ucchoice===true)
      {
          var resp = await fetch(`${process.env.REACT_APP_APIURL}/deletesubcat/${id}`,
          {
              method:"delete"
          })
          if(resp.ok)
          {
              ...
              {
                  toast.info("Sub Category not Deleted");
              }
          }
          else
          {
            ...
            <div className="login">
                <div className="container">
                    ...
                    <div className="login-form-grids animated wow slideInUp" data-wow-delay=".5s">
                        ...
                        <input type="text" name="scatname" onChange={(e)=>setsubcat(e.target.value)} placeholder="Sub Category Name" required=" " /><br />
                                        ...
                                        </tr>
                                        {
                                            subcatlist.map((data,i) =>
                                                <tr key={i}>
                                                    ...
                                                    <td><button className="btn btn-danger" onClick={() => delsubcat(data._id)}>Delete</button></td>
                                                </tr>
                                            )
                                        }
                                    </tbody>
